...
### step3. 
save_dir에 나의 구글 드라이브에 모델을 저장하고 싶은 경로를 입력하세요. 참고로 모델은 2000step마다 저장이 되며, 디렉토리는 따로 설정하지 않을 경우에 checkpoints에 저장이 됩니다. checkpoints는 /content/diff-svc/checkpoints에 있으며 런타임이 종료되면 날아갈 수 있으니 주의하세요.
...
preprocessed_data_location에 가수이름_binary_data.7z 파일의 경로를, config_location에는 **config_nsf.yaml 파일의 경로**를 입력하세요.
...
### step3.
save_dir에는 내가 모델을 저장할 경로, ckpt_directory에는 가장 최근의 ckpt 파일의 경로를 입력합니다. 데이터 셋의 크기가 1시간 미만으로 작을 때는 endless_ds를 체크합니다.
